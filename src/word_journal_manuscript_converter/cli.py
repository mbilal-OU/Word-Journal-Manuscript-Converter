...
from .branding import DISPLAY_VERSION
...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument(
        "--version",
        action="version",
        version=f"%(prog)s {DISPLAY_VERSION} (engine {__version__})",
    )
    ...
    nav_copy_p.add_argument(
        "--static-review-copy",
        action="store_true",
        help="Explicitly create a separate static review copy from a live citation-manager manuscript.",
    )

    analyze_p = sub.add_parser("analyze", help="Run combined manuscript analysis")
    analyze_p.add_argument("docx")
    analyze_p.add_argument("--profile")
    analyze_p.add_argument("--json-out")
    analyze_p.add_argument("--html-out")
    ...
    template_inspect_p = sub.add_parser(
        "template-inspect",
        help="Inspect transferable formatting in a Word .docx/.dotx journal template",
    )
    template_inspect_p.add_argument("template")
    template_inspect_p.add_argument("--json-out")

    template_retarget_p = sub.add_parser(
        "template-retarget",
        help="Apply safe formatting from a Word journal template to a new manuscript copy",
    )
    template_retarget_p.add_argument("docx")
    template_retarget_p.add_argument("--template", required=True)
    template_retarget_p.add_argument("--output", required=True)
    template_retarget_p.add_argument("--report")
    ...
    update_p = sub.add_parser("check-update", help="Check GitHub Releases for a newer product build")
    update_p.add_argument("--json-out")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        if args.command == "template-inspect":
            _dump(inspect_template(args.template), args.json_out)
            return 0
        if args.command == "template-retarget":
            report = retarget_from_template(args.docx, args.output, args.template)
            _dump(report.to_dict(), args.report)
            return 0 if report.passed else 2
