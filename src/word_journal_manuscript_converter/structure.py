...
def _looks_heading(text: str, style: str | None) -> bool:
    ...
    # Word documents sometimes carry a Heading style on ordinary prose or even
    # bibliography entries. Treat the style as strong evidence, not absolute
    # truth, so one accidental style assignment cannot truncate manuscript
    # structure or the reference list.
    if style and re.match(r"(?i)^heading\s*\d*$", style.replace("_", " ")):
        if len(normalized) <= 220 and len(normalized.split()) <= 32:
            return True
        return False
...
def extract_structure(path: str | Path) -> ManuscriptStructure:
    ...
    if ref_idx is not None:
        # Reference entries are frequently generated by citation managers and may
        # inherit unexpected Word styles. Do not stop merely because a later
        # paragraph is styled as a heading. Stop only at a small set of explicit
        # post-reference section labels.
        post_reference_sections = {
            "appendix", "appendices", "supplementary material", "supplementary information",
            "supporting information", "author contributions", "author contribution statement",
            "acknowledgments", "acknowledgements", "funding", "data availability",
            "data availability statement", "competing interests", "conflict of interest",
            "conflicts of interest", "ethics approval",
        }
        for rec in records:
            if rec.index <= ref_idx:
                continue
            normalized = rec.text.lower().rstrip(":").strip()
            if rec.is_heading and normalized in post_reference_sections:
                break
            if rec.text:
                refs.append(rec)
